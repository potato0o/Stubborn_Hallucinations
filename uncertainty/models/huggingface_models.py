...
class HuggingfaceModel(BaseModel):
    ...
    def __init__(self, model_name, stop_sequences=None, max_new_tokens=None, token=None):
        ...
        if 'llama' in model_name.lower():
            ...
            if 'Llama-2' in model_name:
                ...
                base = 'meta-llama'
                # Llama-3 usually doesn't need -hf suffix on HF hub if using meta-llama/Meta-Llama-3-8B
            else:
                base = 'huggyllama'

            # Load tokenizer with retry logic for rate limiting
            max_retries = 5
            tokenizer_loaded = False
            for attempt in range(max_retries):
                try:
                    self.tokenizer = AutoTokenizer.from_pretrained(
                        f"{base}/{model_name}", token=token)
                    logging.info(f"Loaded tokenizer for {base}/{model_name}")
                    ...
                    if "429" in error_str or "Too Many Requests" in error_str:
                        if attempt < max_retries - 1:
                            ...
                            logging.warning(
                                f"Rate limited when loading tokenizer (attempt {attempt + 1}/{max_retries}). "
                                f"Retrying in {wait_time:.2f} seconds..."
                            )
                            time.sleep(wait_time)
                        else:
                            logging.error("Max retries reached for tokenizer loading.")
                            raise
                    else:
            ...
            if ('7b' in model_name or '13b' in model_name or '8B' in model_name or '8b' in model_name) or eightbit:
                logging.info(f"Loading model {base}/{model_name}...")
                self.model = AutoModelForCausalLM.from_pretrained(
                    f"{base}/{model_name}", device_map="auto",
                    max_memory={0: '23GIB'}, torch_dtype=torch.float16, token=token, **kwargs,)
                logging.info(f"Loaded model {base}/{model_name}")
                ...
                config = AutoConfig.from_pretrained(f"{base}/{model_name}", token=token)
        ...
        # Suppress max_length warning when using max_new_tokens
    ...
    def predict(self, input_data, temperature, return_full=False):
        ...
        if full_answer.startswith(input_data):
            ...
            # Fallback: decode the input tokens to see what they look like in the output
            input_token_len = inputs['input_ids'].shape[1]
            decoded_input = self.tokenizer.decode(outputs.sequences[0][:input_token_len], skip_special_tokens=True)
            if full_answer.startswith(decoded_input):
                 input_data_offset = len(decoded_input)
            elif full_answer.strip().startswith(input_data.strip()):
                 input_data_offset = full_answer.find(input_data.strip()) + len(input_data.strip())
            else:
                 logging.warning(f"Input data not found at start of full_answer.\nInput: {input_data[:50]}...\nFull: {full_answer[:50]}...")
                 # Last resort check: usually the first N tokens overlap
                 input_data_offset = len(decoded_input)
        ...
        if n_generated == 0:
            logging.debug('Only stop_words were generated. For likelihoods and embeddings, taking stop word instead.')
            n_generated = 1
        ...
        if len(log_likelihoods) == 1:
            logging.debug('Taking first and only generation for log likelihood!')
            log_likelihoods = log_likelihoods
        else:
            log_likelihoods = log_likelihoods[:n_generated]

        if len(log_likelihoods) == self.max_new_tokens:
            logging.debug('Generation interrupted by max_token limit.')
