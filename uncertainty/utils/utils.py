...
def get_parser():
    ...
    parser.add_argument(
        ...
        choices=['squad', 'llm', 'llm_gpt-3.5', 'llm_gpt-4', 'bertscore'],
    ...
    parser.add_argument(
        "--num_samples", type=int, default=int(1e9),
        help="Number of samples to use")
    parser.add_argument(
        "--num_few_shot", type=int, default=5,
        help="Number of few shot examples to use")
    parser.add_argument(
        "--num_perturbations", type=int, default=1,
        help="Number of perturbation prompts to use for gradient calculation.")
    parser.add_argument(
        "--gradient_target", type=str, default='last_transformer_block',
        choices=['lm_head', 'layer_norm', 'last_transformer_block', 'middle_transformer_block'],
        help="Layer parameter to compute gradients for (gradient method).")
    parser.add_argument(
        "--use_key_phrase_masking", default=False,
        action=argparse.BooleanOptionalAction,
        help="Use NLTK to extract key phrase and mask gradients for other tokens.")
    parser.add_argument(
        "--use_paraphrase_perturbation", default=False,
        action=argparse.BooleanOptionalAction,
        help="Use paraphrasing prompts to rephrase questions instead of adding prefix/suffix prompts.")
    parser.add_argument(
        "--use_embedding_noise_perturbation", default=False,
        action=argparse.BooleanOptionalAction,
        help="Inject small Gaussian noise into input embeddings instead of textual perturbations.")
    parser.add_argument(
        "--embedding_noise_epsilon", type=float, default=0.1,
        help="Standard deviation of Gaussian noise to inject into input embeddings (default: 0.1).")
    parser.add_argument(
        "--use_mc_dropout_perturbation", default=False,
        action=argparse.BooleanOptionalAction,
        help="Use MC Dropout (stochastic weight masking) as perturbation method instead of input/embedding perturbation.")
    parser.add_argument(
        "--mc_dropout_rate", type=float, default=0.1,
        help="Dropout probability for MC Dropout perturbation (default: 0.1).")
    parser.add_argument(
...
def init_model(args, token=None):
    mn = args.model_name
...
def get_bert_score_metric():
    ...
    def metric(response, example, *args, **kwargs):
        ...
        # We take the max score against any reference answer
        P, R, F1 = scorer.score([response] * len(correct_answers), correct_answers)
        f1_score = F1.max().item()
        logging.info(f'BERTScore F1: {f1_score}, Response: {response}, Correct: {correct_answers}')
        return 1.0 if f1_score > 0.9 else 0.0
...
def save(object, file):
    if not os.path.exists(wandb.run.dir):
